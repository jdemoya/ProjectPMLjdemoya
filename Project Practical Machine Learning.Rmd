...
Next I removed the NA's values and I look for variables with zero variance because they are not good predictors, but there isn't for this set
...
for (i in 7:n){
  if (is.factor(training1[,i]) & i!=160){  
    ...
    testing[testing[,i] == "",i] <- "0";
    testing[testing[,i] == "#DIV/0!",i] <- "0";    
...
The variables that are vey correlated that must be excluded of the model are obtained from but first We must do a preprocess because the factor variables don't work with correlations 
...
Next we get the relevant fields and the results are 34 fields for building the predictive model
...
Then we build the model using Random Forests (rf) initially
...
Then I predict the classe variable in the testing set 
